...
let remoteBuffer = execSync('git show origin/main:src/pages/SuperAdminPage.tsx');
let remote = remoteBuffer.toString('utf8');
let local = fs.readFileSync('src/pages/SuperAdminPage.tsx', 'utf8');
...
if (oRemote && nRemote) {
  ...
  if (oLocal && nLocal) {
    ...
    fs.writeFileSync('src/pages/SuperAdminPage.tsx', local, 'utf8');
